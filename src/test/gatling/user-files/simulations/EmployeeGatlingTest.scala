...
class EmployeeGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Employee entity")
        ...
        .repeat(2) {
            ...
            .get("/vmsmicroservice/api/employees")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new employee")
            .post("/vmsmicroservice/api/employees")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "name":"SAMPLE_TEXT", "designation":"SAMPLE_TEXT", "department":"SAMPLE_TEXT", "email":"SAMPLE_TEXT", "mobile":"SAMPLE_TEXT"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_employee_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created employee")
                .get("/vmsmicroservice${new_employee_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created employee")
            .delete("/vmsmicroservice${new_employee_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
